...
class Dataset_Solar_irradiance(Dataset):
    def __init__(self, root_path, flag='train', size=None,
                 features='S', data_path='ETTh1.csv',
                 target='OT', scale=True, timeenc=0, freq='h', seasonal_patterns=None):
        self.seq_len = size[0]
    ...
    def __read_data__(self):
        ...
        df_raw = np.stack(df_raw, 0)
        df_raw = df_raw[:, 10000:10440]
        df_raw = pd.DataFrame(df_raw.T)
...
class Dataset_Ozone(Dataset):
    ...
    def __read_data__(self):
        ...
        # Fill missing values using the average of adjacent values
        for col in df_raw.columns:
            for i in range(1, len(df_raw) - 1):
                if pd.isna(df_raw.loc[i, col]):
                    if not pd.isna(df_raw.loc[i - 1, col]) and not pd.isna(df_raw.loc[i + 1, col]):
                        df_raw.loc[i, col] = (df_raw.loc[i - 1, col] + df_raw.loc[i + 1, col]) / 2
        ...
        df_raw = df_raw.iloc[2275:2275 + 85, :]
        num_train = 60
        num_test = 25
        border1s = [0, num_train - self.seq_len]
        border2s = [num_train, num_train + num_test]
